...
- In new Turkish reports, keep the visible report surface in natural Türkiye Türkçesi: `Rol Türü`, `Kısa Özet`, `Çalışma Modeli`, `Ücret`, `Tarih`, `Puan`, `İlan URL’si`, `Pipeline ID`, `Şehir`, `İlan Dili`, `Çalışma Türü`, `Maaş Bilgisi`, `Kaynak`, `Güven Düzeyi`.
- Keep internal tracker statuses, pipeline note tags, and `data/tr-listings.jsonl` values canonical (`remote`, `hybrid`, `transparent`, `high`, etc.). Parsers remain backward-compatible with legacy English report keys.
