 ...
 *
 * Maps all non-canonical statuses to canonical ones per tracker-status-registry.json:
...
import { normalizeTrackerStatus } from './tracker-status-utils.mjs';
...
for (let i = 0; i < lines.length; i++) {
  ...
  const result = normalizeTrackerStatus(rawStatus);
...
if (unknowns.length > 0) {
  ...
  console.log('\n❌ Refusing to normalize with unknown or listing-layer statuses present.');
  process.exit(1);
}
